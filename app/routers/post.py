...
async def get_posts(db: Session = Depends(get_db), current_user : int = Depends(oauth2.get_current_user), limit: int = 10, skip: int = 0, search: Optional[str] = ""):  # %20 means space in the url
...
@router.get("/{id}", response_model = schemas.PostOut)   #here the {id} refers to a path parameter
def get_post(id : int, response : Response, db : Session = Depends(get_db) ):       #(id:int) = this is going to validate that it can be converted into a integer and it automatically convert the integer for us.
...
def update_post(id:int, updated_post: schemas.PostCreate, db : Session = Depends(get_db), user_id : int = Depends(oauth2.get_current_user)):
    ...
    if post == None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail = f"post was id: {id} was not found")
